...
export default function CardToolbar<TData>({ table, query, setQuery }: DataTableToolbarProps<TData>) {
  const { dialogs, setDialogState } = useDialogStore();
  const rowsSelected = table.getSelectedRowModel().rows;
  const mutation = useCardDelete();
  ...
  return (
    <div className="flex items-center justify-between">
      <div className="flex flex-1 items-center space-x-2">
        <Dialog
          onOpenChange={(open) => setDialogState(KeyDialogs.card, { open })}
          open={dialogs[KeyDialogs.card]?.open}
        >
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Thêm mới thẻ</DialogTitle>
              <DialogDescription>Điền giá trị vào mẫu để tạo thẻ.</DialogDescription>
            </DialogHeader>
            <CardForm />
          </DialogContent>
        </Dialog>
